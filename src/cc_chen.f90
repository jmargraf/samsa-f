...
!#############################################
subroutine calc_Eccd_chen(CCD,frac_occ)
...
!$OMP END PARALLEL
  ...
  elseif(spins == 2 .and. frac_occ)then
    write(*,*) "    "
    write(*,*) "    Calculating frac. occ. UHF-(L)CCD correlation energy (Garnet Chan Version)"
    write(*,*) "    "
...
!           [ + 0.5 ] * Sum ( p5 p6 ) * t ( p5 p6 h1 h2 ) * v ( p3 p4 p5 p6 ) done
            do c = MOZero,nmo
              ...
              do d = MOZero,c-1
                if (SO_Occ(d) == 1.0) cycle
                occ_factor = (1.0d0-SO_Occ(a))*(1.0d0-SO_Occ(b))   !((1.0d0-SO_Occ(a))*(1.0d0-SO_Occ(b))*(1.0d0-SO_Occ(c))*(1.0d0-SO_Occ(d)))
...
!           [ + 0.5 ] * Sum ( h5 h6 ) * t ( p3 p4 h5 h6 ) * v ( h5 h6 h1 h2 ) done
            do k = MOZero,nmo
              ...
              do l = MOZero,k-1
                if (SO_Occ(l) == 0.0) cycle
                occ_factor = (SO_Occ(i))*(SO_Occ(j)) !*SO_Occ(i)*(SO_Occ(j)))
...
end subroutine calc_Eccd_chen

end module module_cc_chen
